...
describe('LinkCustomerEsgMetricsUseCase', () => {
  it('replaces links with unique metric ids', async () => {
    ...
    await useCase.execute(CUSTOMER_ID, OWNER, [METRIC_A, METRIC_B, METRIC_A]);
  ...
  it('unlinks every metric when metric_ids is empty', async () => {
    ...
    await useCase.execute(CUSTOMER_ID, OWNER, []);
  ...
  /*
   * `customer: null` is what the scoped repository returns both for an unknown
   * id and for a customer owned by another account, so this one case covers
   * the isolation rule for POST /customers/:id/esg-metrics as well.
   */
  it('rejects a customer that is unknown or owned by another account', async () => {
    ...
    await expect(
      useCase.execute(CUSTOMER_ID, OWNER, [METRIC_A]),
    ).rejects.toThrow(CustomerNotFoundError);
    expect(replaceAll).not.toHaveBeenCalled();
  });

  it('looks the customer up scoped to the authenticated owner', async () => {
    const { useCase, findOne } = buildUseCase();
